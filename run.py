...
from orchestration.pools import cancel_run, run_planner, run_reconciler, run_worker
...
def build_parser() -> argparse.ArgumentParser:
    ...
    mode.add_argument("--run-next", action="store_true", help="claim and run the oldest planned, ready task")
    mode.add_argument("--plan-next", action="store_true", help="claim and plan the oldest queued issue")
    ...
    mode.add_argument("--worker", action="store_true", help="continuously claim planned, ready tasks")
    mode.add_argument("--planner", action="store_true", help="continuously analyze queued issues")
    mode.add_argument("--reconciler", action="store_true", help="continuously recover expired leases")
    mode.add_argument("--reconcile-once", action="store_true", help="recover expired leases once and exit")
    parser.add_argument("--loop", type=int, default=0, metavar="N", help="run N autonomous rounds")
    ...
    parser.add_argument("--planner-max-runs", type=int, default=0, metavar="N")
    parser.add_argument("--planner-idle-sleep", type=int, default=PLANNER_IDLE_SLEEP, metavar="SECONDS")
    parser.add_argument("--reconciler-interval", type=int, default=RECONCILER_INTERVAL, metavar="SECONDS")
    return parser
...
def _validate_args(parser: argparse.ArgumentParser, args):
    if args.loop > 0 and any((
        ...
        args.reconciler, args.reconcile_once,
    )):
    ...
    if args.worker_idle_sleep <= 0 or args.planner_idle_sleep <= 0 or args.reconciler_interval <= 0:
        parser.error("worker, planner, and reconciler intervals must be positive")
